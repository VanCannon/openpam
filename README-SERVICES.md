...
This document provides an overview of all services implemented for the OpenPAM system.
...
- **License Service** (Port 8086): License validation and feature flags
- **Scheduling Service** (Port 8081): Time-based access control
- **Identity Service** (Port 8082): AD/LDAP synchronization
- **Activity Service** (Port 8083): User lifecycle management and script execution
- **Automation Service** (Port 8084): Ansible playbook execution
- **Communications Service** (Port 8085): Email, Slack, Teams, SIEM integration
...
```bash
cd license
go run cmd/license/main.go
```

## Agent Details

### 1. License Service (Port 8086)
...
### 2. Scheduling Service (Port 8081)
...
### 5. Automation Service (Port 8084)
...
### 6. Communications Service (Port 8085)
