...
import (
	...
	"github.com/openpam/license-agent/internal/config"
	"github.com/openpam/license-agent/internal/database"
	"github.com/openpam/license-agent/internal/events"
	"github.com/openpam/license-agent/internal/handlers"
...
func main() {
	...
	log.Info("Starting License Agent", map[string]interface{}{
	...
	// Initialize service
	svc := license.NewService(db.DB(), log)
	...
	subscriber, err := events.NewSubscriber(cfg.NATS.URL, svc, log)
	if err != nil {
		log.Fatal("Failed to create NATS subscriber", map[string]interface{}{
			"error": err.Error(),
		})
	}
	...
	if err := subscriber.Start(); err != nil {
		log.Fatal("Failed to start NATS subscriber", map[string]interface{}{
			"error": err.Error(),
		})
	}
	...
	mux.HandleFunc("/api/v1/license/validate", handler.ValidateLicense)
	mux.HandleFunc("/api/v1/license/usage", handler.GetUsageStats)
	mux.HandleFunc("/api/v1/license/feature", handler.CheckFeature)
	mux.HandleFunc("/api/v1/license", handler.GetLicense)
